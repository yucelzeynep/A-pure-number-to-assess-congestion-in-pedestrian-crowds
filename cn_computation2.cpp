...
double T_MIN=1e10; //tracking area corners, assume tracking area rectangular parallell to axes
double T_MAX=-1e10;
double X_MIN=1e10;
double Y_MIN=1e10;
double X_MAX=-1e10;
double Y_MAX=-1e10; //initialises with extreme values, then searches in data
...
double XI_MIN,YI_MIN,YI_MAX,XI_MAX;//Interest area corners, decided by user
...
int Init_images();
int dpxi;   //size in pixels of a cell for graphics
int PIXEL_X;  //size of 2D images
int PIXEL_Y;   //size of 2D images
...
public:
  ...
  double delta_t,delta_t_2;//time steps for cn computation
  double mint,maxt;//maximum and minimum time
  int time_steps;//lenght of time vector
  int ripet;//number of repetitions
  DDistr(){}
  DDistr(int r,double mnt,double mxt,double dt)//intialises giving # of reps, minimum time, maximum time and time step
  {
    ripet=r;
    mint=mnt;
    maxt=mxt;
    delta_t=dt;
    delta_t_2=delta_t/2;    
    time_steps=int((maxt-mint)/delta_t);
    d=new Stat[time_steps];//allocates
    dd=new Stat*[ripet];
    for(int i=0;i<ripet;i++) dd[i]=new Stat[time_steps];
    max=-1e10;//very negative max initialisation
  }
  void Init(int r,double mnt,double mxt,double dt)//intialises giving # of reps, minimum time, maximum time and time step, as above
  {
    ripet=r;
    mint=mnt;
    maxt=mxt;
  ...
  void Print(char name[200])//prints the vector on a file with name
  {
    ofstream out;
    out.open(name);
    for(int i=0;i<time_steps;i++) out << mint+i*delta_t+delta_t_2 << " " << d[i].av-d[i].er << " " << d[i].av << " " << d[i].av+d[i].er << endl;
    out.close();
  }
};
...
public:
  ...
  double xL,yL;\\size of the rectangular area
  double T_max,T_min;//maximum time
  CN ****cells;//the fields in reps,time,x,y
  bool **in_area;  //false if cell never used, used to define in area cells (cells with data)
  DDistr dens;//for average density stats
  DDistr dens_interest;//for average density stats  
  ...
  double dp;//pixel size of cell for area
  double graph_mr,graph_md;//for graphycs, max dens and max rotor
  double XI_MIN,YI_MIN,XI_MAX,YI_MAX,CA;//extreme point and area of interest zone
  int imin,imax,jmin,jmax;
  int PIXEL_X,PIXEL_Y;
  Cells(){}   
  void Init(double s,double dt,double xmin,double xmax,double ymin,double ymax,double Tmin,double Tmax,double xim,double yim,double ximx,double yimx,int r,double cnr,double gmr,double gmd,int ext_dp)//initialises
  {
    step=s;
    step2=step*2;
    ...
    T_max=Tmax;
    T_min=Tmin;
    time_steps=int((T_max-T_min)/delta_t);
    x_min=xmin;
    x_max=xmax;
    y_min=ymin;
    y_max=ymax;    
    ...
    dens.Init(ripet,T_min,T_max,delta_t);//and inits distributions
    dens_interest.Init(ripet,T_min,T_max,delta_t);//and inits distributions    
    max_cn.Init(ripet,T_min,T_max,delta_t);
    av_cn.Init(ripet,T_min,T_max,delta_t);
    av_in_cn.Init(ripet,T_min,T_max,delta_t);
    cn_radius=cnr;
    ...
    XI_MAX=ximx;
    YI_MAX=yimx;
    CA=(YI_MAX-YI_MIN)*(XI_MAX-XI_MIN);
    PIXEL_X=dpxi*int(xL/step);
    PIXEL_Y=int(PIXEL_X*yL/xL);
    dp=xL/PIXEL_X;    
    imin=int((XI_MIN-x_min)/xL*PIXEL_X);
    imax=int((XI_MAX-x_min)/xL*PIXEL_X);
    jmin=int((YI_MIN-y_min)/yL*PIXEL_Y);
    jmax=int((YI_MAX-y_min)/yL*PIXEL_Y);
  }
  ...
  {
    if((t>=T_min)&&(t<T_max)&&(r.x>=x_min)&&(r.y>=y_min)&&(r.x<x_max)&&(r.y<y_max)) return true;
    else return false;
  }
  void Up_all(int s,double t)//updates number of events in each cell/distribution
  {
    if((t<T_max)&&(t>=T_min))
      {
	int it=int((t-T_min)/delta_t);
  ...
  void Final_dens()//computes average v field and density in central area
  {
      ...
      for(int i=0;i<time_steps;i++)//density is defined as the average over used cells
	{
	  for(int j=0;j<size_x;j++)
	    for(int k=0;k<size_y;k++)
	      {
		if(in_area[j][k]) dens.dd[s][i].Update(cells[s][i][j][k].dens);
		if(Check_in_middle(j*step+X_MIN,k*step+Y_MIN)) dens_interest.dd[s][i].Update(cells[s][i][j][k].dens);
	      }
	  dens.dd[s][i].Final();
	  dens_interest.dd[s][i].Final();	  
	}
  }
  void Rotor()//computes rotor
  {
  ...
  void Print_v(char name_table[200],char name_graph[200],int r,int t)//prints velocity field
  {
	...
	{
	  DrawPixel(scene,i,j,255,255,255);
	}
    for(int ix=0;ix<size_x;ix++)
      ...
      for(int j=0;j<size_y;j++)
	{
     ...
     for(int i=0;i<PIXEL_X;i++)
      for(int j=0;j<PIXEL_Y;j++)
	{
	  if((abs(i-imin)<2)&&(j>=jmin)&&(j<=jmax)) DrawPixel(scene,i,j,0,0,0);
	  if((abs(i-imax)<2)&&(j>=jmin)&&(j<=jmax)) DrawPixel(scene,i,j,0,0,0);
	  if((abs(j-jmin)<2)&&(i>=imin)&&(i<=imax)) DrawPixel(scene,i,j,0,0,0);
	  if((abs(j-jmax)<2)&&(i>=imin)&&(i<=imax)) DrawPixel(scene,i,j,0,0,0);	  
	}     
    DrawIMG(scene,0,0); 
    SDL_Flip(screen);
  ...
  bool Check_in_middle(double x,double y)//checks if inside central area
  {
    if((x>=XI_MIN)&&(y>=YI_MIN)&&(x<XI_MAX)&&(y<YI_MAX)) return true;
    else return false;
  }
  void Draw_out()//density field
  {   
    for(int i=0;i<PIXEL_X;i++)
      for(int j=0;j<PIXEL_Y;j++)
	{
	  double x=i*dp+x_min;
	  double y=j*dp+y_min;
	  if(Check_in_middle(x,y)) DrawPixel(scene,i,j,255,255,255);
	  else DrawPixel(scene,i,j,255,255,0);
	  if((abs(i-imin)<2)&&(j>=jmin)&&(j<=jmax)) DrawPixel(scene,i,j,0,0,0);
	  if((abs(i-imax)<2)&&(j>=jmin)&&(j<=jmax)) DrawPixel(scene,i,j,0,0,0);
	  if((abs(j-jmin)<2)&&(i>=imin)&&(i<=imax)) DrawPixel(scene,i,j,0,0,0);
	  if((abs(j-jmax)<2)&&(i>=imin)&&(i<=imax)) DrawPixel(scene,i,j,0,0,0);	  
	  ...
	  if(!in_area[i][j])
	    {
	      int ix=i*dpix+dpix_2;
	      int iy=j*dpix+dpix_2;
	      DrawBallnp(scene,0,0,0,dpix_2,ix,iy,PIXEL_X,PIXEL_Y);
	    }
	}
  }
  void Print()//prints everything to files and graphs
  {
    ...
    char name[200];
    char name_graph[200];
    sprintf(name,"data2/R_%f_T_%f_L_%f/dens_time.dat",grid_size,time_int,cn_radius);
    dens.Print(name);
    sprintf(name,"data2/R_%f_T_%f_L_%f/dens_interest_time.dat",grid_size,time_int,cn_radius);
    dens_interest.Print(name);    
    sprintf(name,"data2/R_%f_T_%f_L_%f/max_cn_time.dat",grid_size,time_int,cn_radius);
    max_cn.Print(name);
    sprintf(name,"data2/R_%f_T_%f_L_%f/av_cn_time.dat",grid_size,time_int,cn_radius);
    av_cn.Print(name);
    sprintf(name,"data2/R_%f_T_%f_L_%f/av_in_cn_time.dat",grid_size,time_int,cn_radius);
    av_in_cn.Print(name);    
    sprintf(name,"data2/R_%f_T_%f_L_%f/tables/max_dens_v.dat",grid_size,time_int,cn_radius);
    sprintf(name_graph,"data2/R_%f_T_%f_L_%f/graphs/max_dens_v.bmp",grid_size,time_int,cn_radius);    
    Print_v(name,name_graph,dens.imr,dens.imt);
    sprintf(name,"data2/R_%f_T_%f_L_%f/tables/max_dens_rot.dat",grid_size,time_int,cn_radius);
    sprintf(name_graph,"data2/R_%f_T_%f_L_%f/graphs/max_dens_rot.bmp",grid_size,time_int,cn_radius);
    Print_rot(name,name_graph,dens.imr,dens.imt);
    sprintf(name,"data2/R_%f_T_%f_L_%f/tables/max_dens_cn.dat",grid_size,time_int,cn_radius);
    sprintf(name_graph,"data2/R_%f_T_%f_L_%f/graphs/max_dens_cn.bmp",grid_size,time_int,cn_radius);
    Print_cn(name,name_graph,dens.imr,dens.imt);
    sprintf(name,"data2/R_%f_T_%f_L_%f/tables/max_dens_dens.dat",grid_size,time_int,cn_radius);
    sprintf(name_graph,"data2/R_%f_T_%f_L_%f/graphs/max_dens_dens.bmp",grid_size,time_int,cn_radius);
    Print_dens(name,name_graph,dens.imr,dens.imt);    
    sprintf(name,"data2/R_%f_T_%f_L_%f/tables/max_dens_interest_v.dat",grid_size,time_int,cn_radius);
    sprintf(name_graph,"data2/R_%f_T_%f_L_%f/graphs/max_dens_interest_v.bmp",grid_size,time_int,cn_radius);    
    Print_v(name,name_graph,dens_interest.imr,dens_interest.imt);
    sprintf(name,"data2/R_%f_T_%f_L_%f/tables/max_dens_interest_rot.dat",grid_size,time_int,cn_radius);
    sprintf(name_graph,"data2/R_%f_T_%f_L_%f/graphs/max_dens_interest_rot.bmp",grid_size,time_int,cn_radius);
    Print_rot(name,name_graph,dens_interest.imr,dens_interest.imt);
    sprintf(name,"data2/R_%f_T_%f_L_%f/tables/max_dens_interest_cn.dat",grid_size,time_int,cn_radius);
    sprintf(name_graph,"data2/R_%f_T_%f_L_%f/graphs/max_dens_interest_cn.bmp",grid_size,time_int,cn_radius);
    Print_cn(name,name_graph,dens_interest.imr,dens_interest.imt);
    sprintf(name,"data2/R_%f_T_%f_L_%f/tables/max_dens_interest_dens.dat",grid_size,time_int,cn_radius);
    sprintf(name_graph,"data2/R_%f_T_%f_L_%f/graphs/max_dens_interest_dens.bmp",grid_size,time_int,cn_radius);
    Print_dens(name,name_graph,dens_interest.imr,dens_interest.imt);
    sprintf(name,"data2/R_%f_T_%f_L_%f/tables/max_cn_v.dat",grid_size,time_int,cn_radius);
    sprintf(name_graph,"data2/R_%f_T_%f_L_%f/graphs/max_cn_v.bmp",grid_size,time_int,cn_radius);    
    Print_v(name,name_graph,max_cn.imr,max_cn.imt);
    sprintf(name,"data2/R_%f_T_%f_L_%f/tables/max_cn_rot.dat",grid_size,time_int,cn_radius);
    sprintf(name_graph,"data2/R_%f_T_%f_L_%f/graphs/max_cn_rot.bmp",grid_size,time_int,cn_radius);
    Print_rot(name,name_graph,max_cn.imr,max_cn.imt);
    sprintf(name,"data2/R_%f_T_%f_L_%f/tables/max_cn_cn.dat",grid_size,time_int,cn_radius);
    sprintf(name_graph,"data2/R_%f_T_%f_L_%f/graphs/max_cn_cn.bmp",grid_size,time_int,cn_radius);
    Print_cn(name,name_graph,max_cn.imr,max_cn.imt);
    sprintf(name,"data2/R_%f_T_%f_L_%f/tables/max_cn_dens.dat",grid_size,time_int,cn_radius);
    sprintf(name_graph,"data2/R_%f_T_%f_L_%f/graphs/max_cn_dens.bmp",grid_size,time_int,cn_radius);
    Print_dens(name,name_graph,max_cn.imr,max_cn.imt);        
    sprintf(name,"data2/R_%f_T_%f_L_%f/tables/max_av_cn_v.dat",grid_size,time_int,cn_radius);
    sprintf(name_graph,"data2/R_%f_T_%f_L_%f/graphs/max_av_cn_v.bmp",grid_size,time_int,cn_radius);    
    Print_v(name,name_graph,av_cn.imr,av_cn.imt);
    sprintf(name,"data2/R_%f_T_%f_L_%f/tables/max_av_cn_rot.dat",grid_size,time_int,cn_radius);
    sprintf(name_graph,"data2/R_%f_T_%f_L_%f/graphs/max_av_cn_rot.bmp",grid_size,time_int,cn_radius);
    Print_rot(name,name_graph,av_cn.imr,av_cn.imt);
    sprintf(name,"data2/R_%f_T_%f_L_%f/tables/max_av_cn_cn.dat",grid_size,time_int,cn_radius);
    sprintf(name_graph,"data2/R_%f_T_%f_L_%f/graphs/max_av_cn_cn.bmp",grid_size,time_int,cn_radius);
    Print_cn(name,name_graph,av_cn.imr,av_cn.imt);
    sprintf(name,"data2/R_%f_T_%f_L_%f/tables/max_av_cn_dens.dat",grid_size,time_int,cn_radius);
    sprintf(name_graph,"data2/R_%f_T_%f_L_%f/graphs/max_av_cn_dens.bmp",grid_size,time_int,cn_radius);
    Print_dens(name,name_graph,av_cn.imr,av_cn.imt);   
    sprintf(name,"data2/R_%f_T_%f_L_%f/tables/max_av_in_cn_v.dat",grid_size,time_int,cn_radius);
    sprintf(name_graph,"data2/R_%f_T_%f_L_%f/graphs/max_av_in_cn_v.bmp",grid_size,time_int,cn_radius);    
    Print_v(name,name_graph,av_in_cn.imr,av_in_cn.imt);
    sprintf(name,"data2/R_%f_T_%f_L_%f/tables/max_av_in_cn_rot.dat",grid_size,time_int,cn_radius);
    sprintf(name_graph,"data2/R_%f_T_%f_L_%f/graphs/max_av_in_cn_rot.bmp",grid_size,time_int,cn_radius);
    Print_rot(name,name_graph,av_in_cn.imr,av_in_cn.imt);
    sprintf(name,"data2/R_%f_T_%f_L_%f/tables/max_av_in_cn_cn.dat",grid_size,time_int,cn_radius);
    sprintf(name_graph,"data2/R_%f_T_%f_L_%f/graphs/max_av_in_cn_cn.bmp",grid_size,time_int,cn_radius);
    Print_cn(name,name_graph,av_in_cn.imr,av_in_cn.imt);
    sprintf(name,"data2/R_%f_T_%f_L_%f/tables/max_av_in_cn_dens.dat",grid_size,time_int,cn_radius);
    sprintf(name_graph,"data2/R_%f_T_%f_L_%f/graphs/max_av_in_cn_dens.bmp",grid_size,time_int,cn_radius);
    Print_dens(name,name_graph,av_in_cn.imr,av_in_cn.imt);
    sprintf(name,"data2/R_%f_T_%f_L_%f/final.dat",grid_size,time_int,cn_radius);
    out.open(name);
    out << "Maximum CN=" << max_cn.max << " at t=" << T_min+max_cn.imt*delta_t+delta_t_2 << " ripet=" << max_cn.imr << endl;
    out << "Maximum density=" << dens.max << " at t=" << T_min+dens.imt*delta_t+delta_t_2 << " ripet=" << dens.imr << endl;
    out << "Maximum average CN=" << av_cn.max << " at t=" << T_min+av_cn.imt*delta_t+delta_t_2 << " ripet=" << av_cn.imr << endl;
    out << "Maximum average in area CN=" << av_in_cn.max << " at t=" << T_min+av_in_cn.imt*delta_t+delta_t_2 << " ripet=" << av_in_cn.imr << endl;
    out.close();
  }
};
...
bool Check_in_area(double x,double y)//checks if inside central area
{
  if((x>=XI_MIN)&&(y>=YI_MIN)&&(x<XI_MAX)&&(y<YI_MAX)) return true;
  else return false;
}
...
void Limits(int sim,double &tmin,double &tmax,double &xmin,double &xmax,double &ymin,double &ymax)
{\\to know the rectangular area with data
  char name[200];
  ifstream in;
  for(int i=0;i<sim;i++)
    {
      sprintf(name,"positions/pos_%d.dat",i);
      in.open(name);
      if(in.good())
	{
	  int n;
...
void Read_parameters()//gets environment data and computation parameters
{
  string token;
  ifstream read("parameters2");
  read >> token;
  read >> ripet;  
  read >> token;
  read >> dpxi;
...
void Initialise()
{
  ...
  XL_2=XL/2;
  YL_2=YL/2;
  PIXEL_X=dpxi*int(XL/grid_size);
  PIXEL_Y=dpxi*int(YL/grid_size);
}

double Easy_round_down(double &val)
{
  if(val>=0) val=int(val);
  else val=int(val)-1;
}

double Easy_round_up(double &val)
{
  if(val>=0) val=int(val)+1;
  else val=int(val);
}

double Easy_round_down(double &val,double step)
{
  if(val>=0) val=(int(val/step)-1)*step;
  else val=(int(val/step)-2)*step;
}

double Easy_round_up(double &val,double step)
{
  if(val>=0) val=(int(val/step)+2)*step;
  else val=int(val/step+1)*step;
}

void Names()
{
  char name_check[200];
  sprintf(name_check,"data2/R_%f_T_%f_L_%f",grid_size,time_int,cn_radius);
  if (stat(name_check, &st) == -1) {
    mkdir(name_check, 0700);
  }
  sprintf(name_check,"data2/R_%f_T_%f_L_%f/graphs",grid_size,time_int,cn_radius);
  if (stat(name_check, &st) == -1) {
    mkdir(name_check, 0700);
  }
  sprintf(name_check,"data2/R_%f_T_%f_L_%f/tables",grid_size,time_int,cn_radius);
  if (stat(name_check, &st) == -1) {
    mkdir(name_check, 0700);
  }  
}  
...
{
  char name[200];
  Read_parameters();//reads parameters
  Limits(ripet,T_MIN,T_MAX,X_MIN,X_MAX,Y_MIN,Y_MAX);
  Easy_round_down(T_MIN,time_int);
  Easy_round_down(X_MIN,grid_size);
  Easy_round_down(Y_MIN,grid_size);
  Easy_round_up(T_MAX,time_int);
  Easy_round_up(X_MAX,grid_size);
  Easy_round_up(Y_MAX,grid_size);
  Initialise();  
  Init_images();
  SDL_Inizio();//starts graphics
  Names();   
  CELLS.Init(grid_size,time_int,X_MIN,X_MAX,Y_MIN,Y_MAX,T_MIN,T_MAX,XI_MIN,YI_MIN,XI_MAX,YI_MAX,ripet,cn_radius,graph_mr,graph_md,dpxi);
